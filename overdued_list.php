...
if (!$reportView) {
    ...
    <!-- filter -->
    <div>
        <div class="text-left loan_total" style='color:red;background:yellow' class="per_title">
            <h2><?php echo __('>>DAFTAR TERLAMBAT'); ?></h2>
        </div>
        <div class="infoBox">
          <?php echo __('Report Filter'); ?>
          <div style='color:red;'><?php echo __('Melaporkan anggota yang TERLAMBAT mengembalikan buku. Hal ini bisa ditagihkan melalui notifikasi WA kepada Wali Kelas dan atau Orang Tua/Wali Siswa. Total yang tercantum adalah jumlah orang.-- maseko --'); ?></div> 
        </div>
  ...
  <?php
    ...
    function showOverduedList($obj_db, $array_data)
    {
        ...
        $_buffer = '<div style="font-weight: bold; color: blue; font-size: 12pt; margin-bottom: 3px;">' . $member_name . ' (' . $array_data[0] . ')</div>';
        $_buffer .= '<div style="color: black; font-size: 10pt; margin-bottom: 3px;">' . $member_mail_address . '</div>';
        if (!empty($member_d[1])) $_buffer .= '<div style="font-size: 10pt; margin-bottom: 3px;"><div id="' . $array_data[0] . 'emailStatus"></div>' . __('WA Orang Tua/Wali') . ' : <a href="mailto:' . $member_d[1] . '">' . $member_d[1] . '</a> - <a class="usingAJAX btn btn-sm btn-outline-primary" href="' . MWB . 'membership/overdue_mail.php' . '" postdata="memberID=' . $array_data[0] . '" loadcontainer="' . $array_data[0] . 'emailStatus"><i class="fa fa-paper-plane-o"></i>&nbsp;' . __(' ') . '</a> <br/>';
        $_buffer .= __('WA Wali Kelas') . ': ' . $member_d[2] . '</div>';
        ...
        while ($ovd_title_d = $ovd_title_q->fetch_assoc()) {
            ...
            $_buffer .= '<tr>';
            $_buffer .= '<td valign="top" width="7%">' . $ovd_title_d['item_code'] . '</td>';
            $_buffer .= '<td valign="top" width="30%">' . $ovd_title_d['title'] . '</div></td>';
            // $_buffer .= '<td valign="top" width="40%">' . $ovd_title_d['title'] . '<div>' . __('Book Price') . ': ' . currency($ovd_title_d['price']) . '</div></td>';
            $_buffer .= '<td valign="top" width="12%"><div class="btn btn-sm btn-outline-primary" style="color: red;">' . __(' ') . ' ' . $overdue_days+1 . ' ' . __('hari terlambat') .'</div></td>';
            // $_buffer .= '<td width="15%"><div>' . __('Terlambat') . ': ' . $overdue_days+1 . ' ' . __('day(s)') . '</div><div>'.__('Fines').': '.$fines.'</div></td>';
            $_buffer .= '<td valign="top" width="30%">' . __('Tgl Pinjam') . ': ' . $ovd_title_d['loan_date'] . ' &nbsp; ' . __('Tgl Kembali') . ': ' . $ovd_title_d['due_date'] . '</td>';
                       
            $_buffer .= '<td valign="top" width="10%"><p>WA Wali Kelas <a class="btn btn-sm btn-outline-primary" href="https://wa.me/'.$member_d[2].'?text=Hallooo....%0a%0aSahabat Ruang Perpustakaan SD Marsudirini Yogyakarta, kami memberitahukan bahwa:%0a%0aNama: *_'.$member_d[0].',_* %0aada keterlambatan pinjaman buku *' . $overdue_days+1 . '  hari* dengan:%0a%0aKode Barcode: *'.$ovd_title_d['item_code'].'*%0aJudul: *' . $ovd_title_d['title'] . '*%0aTanggal harus kembali: ' . $ovd_title_d['due_date'] . '%0a'.__('Fines').': '.$fines.' .%0a%0aMohon buku segera dikembalikan ke perpustakaan melalui petugas.%0a%0aTerima Kasih.%0aTuhan memberkati.%0a%0a'.$sysconf['library_name'].'%0a'.$sysconf['library_subname'].'" target="_blank"> <i class="fa fa-paper-plane-o"> '.$member_d[2].''.$ovd_title_d[3].'</a></p></td>'; 
            $_buffer .= '<td valign="top" width="11%"><p>WA Ortu<a class="btn btn-sm btn-outline-primary" href="https://wa.me/'.$member_d[1].'?text=Yang Terkasih%0aBapak/Ibu Orang Tua/Wali Murid%0aSD Marsudirini Yogyakarta,%0a%0aDari Perpustakaan, kami memberitahukan bahwa:%0a%0aNama: *'.$member_d[0].'*%0a%0aAda keterlambatan pinjaman buku *' . $overdue_days+1 . '  hari* dengan:%0a%0aKode Barcode: *'.$ovd_title_d['item_code'].'*%0aJudul: *' . $ovd_title_d['title'] . '*%0aTanggal harus kembali: ' . $ovd_title_d['due_date'] . '%0a'.__('Fines').': '.$fines.'%0a%0aMohon buku bisa segera dikembalikan ke perpustakaan melalui petugas.%0aApabila belum selesai membaca, dapat memperpanjang peminjaman melalui petugas dengan membawa kartu anggota.%0a%0aTerima Kasih.%0aTuhan memberkati.%0a%0a'.$sysconf['library_name'].'-'.$sysconf['library_subname'].'" target="_blank"> <i class="fa fa-paper-plane-o"> '.$member_d[1].''.$ovd_title_d[3].'</a></p></td>';
